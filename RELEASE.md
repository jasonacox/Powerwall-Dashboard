...
## v5.0.7 - Wired LAN (v1r) Setup Fixes

### pyPowerwall Update

* Update pypowerwall to v0.15.8.

### Setup Updates

* Fixed several bugs in `setup.sh` for the **Wired LAN (v1r)** mode introduced in v5.0.6:
  - **RSA key path fix**: Pass `-authpath /app/.auth` to `pypowerwall setup -v1r` so the generated RSA key pair is written directly into the bind-mounted `.auth/` volume. Previously the key was written to `/app/` which is not a persistent volume and is not writable by the non-root container user, causing a `PermissionError`.
  - **Correct `PW_RSA_KEY_PATH` value**: The env file now correctly writes `PW_RSA_KEY_PATH=/app/.auth/tedapi_rsa_private.pem` (the container-absolute path and the actual filename generated by pypowerwall). The previous value used a relative path and the wrong filename.
  - **Credential routing fix**: v1r mode now correctly enters the local access credential collection path. Previously `config="Wired LAN (v1r)"` fell through to the Tesla Cloud email prompt, so `PW_HOST` was never collected on a fresh install.
  - **Missing settings guard**: Before running the RSA registration, the v1r block now checks for and prompts for any missing `PW_HOST`, `PW_GW_PWD`, `PW_RSA_KEY_PATH`, and `PW_WIFI_HOST` — covering re-runs where an existing env file was kept without those settings.
  - **Container env reload**: Replaced `docker restart pypowerwall` with `./compose-dash.sh up -d` after env file changes so the container is recreated and picks up the updated environment variables.
